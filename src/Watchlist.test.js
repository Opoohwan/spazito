...
describe('Watchlist.tickers', () => {
  ...
  test('the cap holds on read: a hand-edited 12-ticker property yields only MAX_TICKERS', () => {
    const warn = jest.spyOn(console, 'warn').mockImplementation(() => {});
    const twelve = ['T0', 'T1', 'T2', 'T3', 'T4', 'T5', 'T6', 'T7', 'T8', 'T9', 'TA', 'TB'];
    props.setProperty('WATCHLIST', JSON.stringify(twelve));
    expect(Watchlist.tickers()).toEqual(twelve.slice(0, Watchlist.MAX_TICKERS));
    expect(warn).toHaveBeenCalled();
  });
